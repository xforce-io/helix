...
test('S1.generation adapter reaches the model only via injected milkie IOPort', async () => {
  ...
  const generated = await adapter.generate({ sourceRunRefs: ['source'], baselineRef, policy, reservedGenerationRunRef: 'recorded-run' })
...
test('S1.generation adapter rejects more than one model text output', async () => {
  ...
  await assert.rejects(
    adapter.generate({ sourceRunRefs: ['source'], baselineRef, policy, reservedGenerationRunRef: 'recorded-run' }),
